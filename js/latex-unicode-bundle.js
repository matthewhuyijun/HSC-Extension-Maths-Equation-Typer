(function (root, factory) {
    if (typeof module === 'object' && module.exports) {
        ...
        const api = factory();
        root.LatexUnicodeMath = api;
        if (typeof root.convertLatexToUnicodeMath !== 'function') {
            root.convertLatexToUnicodeMath = api.convertLatexToUnicodeMath;
        }
        if (!root.LatexConverter) {
            root.LatexConverter = {
                toWordEquation: api.toWordEquation,
                normalizeWordInput: api.normalizeWordInput,
                removeWordSpaces: api.removeWordSpaces
            };
        }
        if (!root.LatexUtils && api.normalizeLatexStr) {
            root.LatexUtils = {
                normalizeLatexStr: api.normalizeLatexStr,
                isEmptyLatex: api.isEmptyLatex,
                extractContent: api.extractContent
            };
        }
    }
...
(function(window) {
    ...
    const LatexConverter = scope.LatexConverter || {};
    const LatexUtils = scope.LatexUtils || {};

    const api = {
        convertLatexToUnicodeMath: latex => (LatexConverter.toWordEquation ? LatexConverter.toWordEquation(latex) : String(latex || '')),
        toWordEquation: LatexConverter.toWordEquation || (latex => String(latex || '')),
        normalizeWordInput: LatexConverter.normalizeWordInput || (text => String(text || '')),
        removeWordSpaces: LatexConverter.removeWordSpaces || (latex => String(latex || '')),
        normalizeLatexStr: LatexUtils.normalizeLatexStr,
        isEmptyLatex: LatexUtils.isEmptyLatex,
        extractContent: LatexUtils.extractContent,
        SymbolMaps: scope.SymbolMaps,
        LatexParser: scope.LatexParser,
        ASTPrinter: scope.ASTPrinter,
        PostProcessor: scope.PostProcessor
    };
